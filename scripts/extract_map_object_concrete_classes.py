...
def main() -> None:
    ...
    map_object_data_table: dict[str | int, Any] = {}
    # Load the map object DataTable
    with open(os.path.join(export_path, "Pal", "Content", "Pal", "DataTable", "MapObject", "DT_MapObjectMasterDataTable.json"), "rb") as f:
        map_object_data_table = json.load(f)
    # Load the blueprints by walking down the export list
    blueprints: dict[str, Any] = {}
    for root, _, files in os.walk(os.path.join(export_path, "Pal", "Content", "Pal", "Blueprint")):
        for file in files:
    ...
    object_blueprints: dict[str, Any] = {}
    ...
    def resolve_concrete_class_name(class_name: str) -> str | None:
        if class_name.startswith("Class'"):
            ...
            for _, class_data in blueprint_class.items():
                if class_data["Type"] == "BlueprintGeneratedClass":
                    return resolve_concrete_class_name(class_data["SuperStruct"]["ObjectName"])
        print(f"Warning: Unhandled class name {class_name}")
        return None

    def resolve_object_super_concrete_class_name(class_name: str) -> str | None:
        if class_name.startswith("Class'"):
            return class_name.removesuffix("'").removeprefix("Class'")
        elif class_name.startswith("BlueprintGeneratedClass'"):
            blueprint_class_search = class_name.removesuffix("_C'").removeprefix("BlueprintGeneratedClass'")
            if blueprint_class_search.lower() not in blueprints:
                print(f"Warning: Blueprint class {blueprint_class_search} not found")
                return None
            blueprint_class = blueprints[blueprint_class_search.lower()]
            struct_data = blueprint_class.get(f"Default__{blueprint_class_search}_C".lower(), None)
            if struct_data is None:
                print("Warning: No struct found for", blueprint_class_search)
                return None
            if "ConcreteModelClass" in struct_data["Properties"]:
                return resolve_concrete_class_name(struct_data["Properties"]["ConcreteModelClass"]["ObjectName"])
        print(f"Warning: Unhandled class name {class_name}")
        return None

    # Extract the concrete classes
    concrete_classes: dict[str, Any] = {}
    for object_name, blueprint_class_name in object_blueprints.items():
        ...
        if struct_data is None:
            ...
            continue
        if "ConcreteModelClass" in struct_data["Properties"]:
            concrete_classes[object_name] = resolve_concrete_class_name(struct_data["Properties"]["ConcreteModelClass"]["ObjectName"])
        else:
            # Search for the super class
            bp_gen_class = blueprint.get(f"{blueprint_class_name}_C".lower(), None)
            if bp_gen_class is None:
                print("Warning: No blueprint generated class found for", blueprint_class_name)
                continue
            ...
            if concrete_class is None:
                ...
                concrete_classes[object_name] = "DEFAULT_UNKNOWN_PalMapObjectConcreteModelBase"
